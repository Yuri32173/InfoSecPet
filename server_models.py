...
# Define the many-to-many relationship between ServerUser and ServerPermission
server_user_permissions = server_db.Table(
    "server_user_permissions",
    server_db.Column("server_user_id", server_db.Integer, server_db.ForeignKey("server_user.id")),
    server_db.Column("server_permission_name", server_db.String(128), server_db.ForeignKey("server_permission.name")),
    info={"bind_key": "server"},
)

class ServerUser(UserMixin, server_db.Model):
    """Model representing server users"""
    __bind_key__ = "server"
    id = server_db.Column(server_db.Integer, primary_key=True)
    username = server_db.Column(server_db.String(32), unique=True, nullable=False)
    ...
    date_created = server_db.Column(server_db.DateTime, default=datetime.datetime.now)
    permissions = server_db.relationship("ServerPermission", secondary=server_user_permissions, backref=server_db.backref("users", lazy="dynamic"))

class ServerPermission(server_db.Model):
    """Model representing server permissions"""
    __bind_key__ = "server"
    name = server_db.Column(server_db.String(128), primary_key=True)

class Request(server_db.Model):
    """Model representing server requests"""
    __bind_key__ = "server"
    id = server_db.Column(server_db.Integer, primary_key=True)
    ...
    request_params = server_db.Column(server_db.String(255), nullable=False)
...
class Alert(server_db.Model):
    """Model representing alerts"""
    __bind_key__ = "server"
    request_id = server_db.Column(server_db.Integer, server_db.ForeignKey("request.id"), primary_key=True)
...
class BackupLog(server_db.Model):
    """Model representing backup logs"""
    __bind_key__ = "server"
    id = server_db.Column(server_db.Integer, primary_key=True)
